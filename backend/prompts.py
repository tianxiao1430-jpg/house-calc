...
{
  "price": <number in yen, NOT man-en. If shown as 3500万円, return 35000000>,
...
- If price shows 3,500万円, return 35000000
...

{
  ...
  "location": <address string, null if not shown>,
  "structure": <"RC", "SRC", "木造", "鉄骨" etc, null if not shown>,
  "deposit_months": <deposit in months (e.g., 1 for 1ヶ月), null if not shown>,
...
def build_chat_prompt(mode: str, extracted: dict) -> str:
    ...
    base = f"""你是在日华人房产顾问AI。用中文友好对话，像朋友聊天。每次只问一个问题。
...
需要确认：头金(默认房价10%)、贷款年限(默认35年)、自住还是投资。
规则：每次只问一个问题。回复简短。用户说"默认"就用默认值。尽快收集完。"""
    else:
        return base + """任务：收集计算所需信息，收集完加 [CALC_READY]。
需要确认：敷金(默认1月)、礼金(默认1月)、是否要保证会社(默认要)。如物件信息里已有就跳过。
规则：每次只问一个问题。回复简短。用户说"默认"就用默认值。尽快收集完。"""
